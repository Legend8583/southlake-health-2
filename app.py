...
STEP_CONFIG = [
    ...
    },
    {
        ...
        "description": "Decide whether to stay in Local Agent mode or escalate to Connected Agent for deeper analysis on the approved synthetic output.",
        "owner": "Data Analyst",
    },
]
...
def inject_styles() -> None:
    st.markdown(
        ...
        <style>
            ...
            .step6-privacy-strip .step6-privacy-label {
                font-size: 0.72rem;
                font-weight: 800;
                letter-spacing: 0.08em;
                text-transform: uppercase;
            ...
            .step6-privacy-strip span + span:not(.step6-privacy-label)::before {
                content: "·";
                margin-right: 0.8rem;
                color: #b8c6d2;
            }
            ...
            /* ── New Local Agent layout (v2 hero) ───────────────────── */
            .step6-mode-tile-v2 {
                background: #ffffff;
            ...
            .step6-mode-tile-v2.is-active {
                border-color: rgba(11, 94, 168, 0.5);
                box-shadow: 0 4px 14px rgba(11, 94, 168, 0.08);
            }

            .step6-mode-tile-v2 .step6-mode-top {
                display: flex;
                align-items: flex-start;
                justify-content: space-between;
                gap: 0.8rem;
            ...
            .step6-mode-tile-v2 .step6-mode-eyebrow {
                color: #6f8297;
                font-size: 0.7rem;
                font-weight: 800;
                text-transform: uppercase;
            ...
            .step6-mode-tile-v2 .step6-mode-name {
                color: #10263f;
                font-size: 1.1rem;
                font-weight: 800;
                line-height: 1.3;
            ...
            .step6-mode-tile-v2 .step6-mode-state {
                flex: 0 0 auto;
            ...
            .step6-mode-tile-v2 .step6-mode-state.is-active {
                background: rgba(11, 94, 168, 0.12);
                color: #0b5ea8;
            }

            .step6-mode-tile-v2 .step6-mode-points {
                list-style: none;
                margin: 0;
                padding: 0;
                display: flex;
            ...
            .step6-mode-tile-v2 .step6-mode-points li {
                display: flex;
                align-items: flex-start;
                gap: 0.5rem;
                color: #27425c;
            ...
            .step6-mode-tile-v2 .step6-mode-points li::before {
                content: "";
                width: 0.3rem;
                height: 0.3rem;
                border-radius: 999px;
                background: rgba(58, 111, 180, 0.65);
                margin-top: 0.48rem;
                flex: 0 0 auto;
            }

            .step6-privacy-footnote {
                display: flex;
                align-items: center;
                justify-content: center;
                gap: 0.5rem;
            ...
            /* ── Agent workspace hero (2-column layout) ─────────────── */
            .step6-hero-kicker {
                color: #6f8297;
                font-size: 0.7rem;
                font-weight: 800;
...
def step_status_labels(metadata: list[dict[str, Any]], controls: dict[str, Any]) -> list[str]:
    ...
    if not st.session_state.results_shared_at:
        ...
        handoff_label = "Connected Agent"
    else:
        handoff_label = "Local Agent"
...
def build_handoff_conditions(
    ...
    conds: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            "label": "Connected Agent approved by reviewer",
...
GUIDANCE_MODE_META = {
    "internal": {
        "label": "Local Agent",
        "short": "Local Agent",
        "tagline": "Default governed analysis mode for workflow reasoning inside the workspace. Fast first-pass readiness, blocker, and release interpretation — fully in-workspace, no external model involved.",
        "color": "#0b5ea8",
        "bg": "#EBF1F7",
        "icon": "",
    },
    "external": {
        ...
        "short": "Connected Agent",
        "tagline": "Deeper analysis on approved synthetic output after governance checks pass. Richer narrative and scenario framing for exploratory downstream analysis. Synthetic-only — no source records ever leave the workspace.",
        "color": "#136B48",
        "bg": "#EDF9F3",
        "icon": "",
    },
...
def build_role_based_guidance(
    ...
    mgr_watch.append("Connected Agent operates on synthetic output only; no source records ever leave the workspace.")
...
def _render_external_payload_panel(
    ...
    status_label = "Connected Agent runtime enabled" if runtime_allowed else "Connected Preview · runtime gated"
    status_detail = (
        "All governance gates have passed. Connected Agent can run on metadata, aggregates, and approved synthetic output only."
        if runtime_allowed
        else "You can preview the Connected workflow now. Connected Agent remains locked until all readiness gates pass. No source records will ever be sent externally."
    )
    cards = [
    ...
    with st.container(border=True, key="guidance_external_payload_panel"):
        st.markdown(
            '<div style="font-size:0.8rem;font-weight:800;text-transform:uppercase;letter-spacing:0.08em;color:#136B48;margin-bottom:0.3rem;">Connected Agent</div>'
            '<div style="font-size:1.15rem;font-weight:700;color:#17324d;margin-bottom:0.2rem;line-height:1.3;">Payload preview &amp; privacy boundary</div>'
            '<div style="font-size:0.82rem;color:#668097;margin-bottom:0.85rem;line-height:1.5;">This is the exact class of information Connected Agent may use: metadata, aggregates, quality metrics, and approved synthetic output. No source records leave the workspace.</div>'
...
def _render_local_analysis_summary(
    ...
    def metric_row(label: str, value: str, tag: str, tag_kind: str) -> str:
        fg, bg, bd = tag_colors.get(tag_kind, tag_colors["neutral"])
        return (
            '<div class="step6-metric-row">'
            f'<div class="step6-metric-row-value">{html.escape(value)}</div>'
            '<div class="step6-metric-row-text">'
            f'<div class="step6-metric-row-label">{html.escape(label)}</div>'
            f'<span class="step6-metric-row-tag" style="color:{fg};background:{bg};border-color:{bd};">{html.escape(tag)}</span>'
            '</div>'
            '</div>'
        )

    # Left column — Package quality metrics stacked vertically + snapshot
    snapshot_bits = []
    snapshot_bits.append(f"{completeness:.1f}% completeness")
    if missing_fields:
        snapshot_bits.append(f"{missing_fields} field(s) with missing values")
    if high_findings or medium_findings:
        snapshot_bits.append(f"{high_findings} high / {medium_findings} medium hygiene flag(s)")
    if restricted_count or sensitive_count:
        snapshot_bits.append(f"{restricted_count} restricted &middot; {sensitive_count} sensitive field(s)")
    ...
    left_html = (
        ...
        + metric_row("Correlation", f"{correlation_score:.1f}", stability_note, correlation_kind)
        + '<div class="step6-hero-divider"></div>'
        '<div class="step6-hero-kicker step6-hero-kicker-sub">Snapshot</div>'
        f'<ul class="step6-hero-snapshot">{snapshot_items}</ul>'
        '</div>'
    )

    # Role briefing (right column)
    ...
    hero_cols = st.columns([0.88, 1.12], gap="large")
    with hero_cols[0]:
...
def _render_chat_module(
    ...
    with st.container(border=True, key="guidance_chat_panel"):
        if mode == "external":
            header_color = "#136B48"
            header_title = "Connected Agent"
            if availability["can_use_external"]:
                header_subtitle = "Deeper analysis on the approved synthetic package"
                header_caption = ("Ask about this package in natural language — scenario framing, narrative "
                                  "interpretation, downstream analysis ideas. The agent sees only metadata, "
                                  "aggregates, quality metrics, and audit context. No source records leave the workspace.")
                banner = (
                    ...
                    'Connected Agent is running on the approved synthetic payload only. '
                    'No source records are sent externally.</div>'
                    '</div>'
                )
            else:
                header_subtitle = "Connected Preview · governance gates still need to pass"
                header_caption = ("Connected mode is selected. Until all readiness gates pass, responses "
                                  "continue through the Local Agent fallback. No source records are ever sent externally.")
                banner = ""
        else:
            header_color = "#0b5ea8"
            header_title = "Local Agent"
            header_subtitle = "Fast in-workspace reasoning &mdash; no external model involved"
            header_caption = ("Responses come from in-workspace templates grounded in the package metadata, "
                              "hygiene findings, and validation state. Escalate to Connected Agent for deeper "
                              "analysis after governance gates pass.")
            banner = ""
        st.markdown(
...
def render_step_six(metadata: list[dict[str, Any]], controls: dict[str, Any]) -> None:
    ...
    if connected_live:
        connected_state = "Connected Agent"
        connected_status = "Connected Agent"
        connected_note = "Live deeper analysis running on the approved synthetic payload. Source records never leave the workspace."
    elif not has_api_key:
        connected_state = "Setup required"
        connected_status = "Setup required"
        connected_note = "Governance gates have passed. Add an API key to activate Connected Agent. Only synthetic output is eligible — no source records leave the workspace."
    else:
        connected_state = "Connected Preview"
        connected_status = "Connected Preview"
        connected_note = "Approved synthetic-only payload is ready. Escalate to Connected Agent to begin live deeper analysis, or stay in Local mode."

    _render_status_strip_v2(5, verdict_label, verdict_color, verdict_bg)

    with st.container(border=True, key="step6_mode_shell"):
        st.markdown(
            '<div class="step6-kicker" style="text-align:center;">Workflow decision</div>'
            '<div class="step6-heading step6-center-heading">Stay local, or escalate to deeper analysis.</div>'
            '<div class="step6-subheading step6-center-subheading">Local is the default governed mode for workflow reasoning. Connected unlocks deeper synthetic-only analysis after all governance gates pass.</div>',
            unsafe_allow_html=True,
        )
        ...
        with mode_cols[0]:
            local_card_html = (
                f'<div class="step6-mode-tile-v2{" is-active" if mode == "internal" else ""}">'
                '<div class="step6-mode-top">'
                '<div>'
                '<div class="step6-mode-eyebrow">Default &middot; Always available</div>'
                '<div class="step6-mode-name">Local Agent</div>'
                '</div>'
                f'<span class="step6-mode-state{" is-active" if mode == "internal" else ""}">'
                f'{"Current" if mode == "internal" else "Available"}</span>'
                '</div>'
                '<ul class="step6-mode-points">'
                '<li>Fast first-pass workflow reasoning</li>'
                '<li>Readiness, blocker, and release interpretation</li>'
                '<li>Fully in-workspace &mdash; no external model involved</li>'
                '</ul>'
                '</div>'
            )
            st.markdown(local_card_html, unsafe_allow_html=True)
            if st.button(
                "Stay in Local mode",
                use_container_width=True,
        ...
        with mode_cols[1]:
            connected_card_html = (
                f'<div class="step6-mode-tile-v2{" is-active" if mode == "external" else ""}">'
                '<div class="step6-mode-top">'
                '<div>'
                '<div class="step6-mode-eyebrow">Downstream &middot; Governance-gated</div>'
                '<div class="step6-mode-name">Connected Agent</div>'
                '</div>'
                f'<span class="step6-mode-state{" is-active" if mode == "external" else ""}">'
                f'{html.escape(connected_state)}</span>'
                '</div>'
                '<ul class="step6-mode-points">'
                '<li>Deeper interpretation of the approved synthetic package</li>'
                '<li>Richer narrative and scenario framing</li>'
                '<li>Synthetic-only &mdash; no source records ever leave the workspace</li>'
                '</ul>'
                '</div>'
            )
            st.markdown(connected_card_html, unsafe_allow_html=True)
            if st.button(
                "Escalate to Connected",
                use_container_width=True,
        ...
        st.markdown(
            '<div class="step6-privacy-footnote">'
            '<span class="step6-privacy-footnote-label">Privacy boundary</span>'
            '<span>No source records sent externally</span>'
            '<span class="step6-privacy-dot">&middot;</span>'
            '<span>Only synthetic output is eligible</span>'
            '<span class="step6-privacy-dot">&middot;</span>'
            '<span>Governance conditions must pass first</span>'
            '</div>',
            unsafe_allow_html=True,
        )
    ...
    if connected_live:
        ...
        workspace_title = "Local Agent &middot; Role console"
        workspace_subheading = "Fast in-workspace reasoning for readiness, blockers, and release decisions."
    else:
        suggested_prompts = []
        workspace_title = "Connected Preview"
        workspace_subheading = (
            "Approved synthetic-only payload is ready. Add an API key to activate Connected Agent for live analysis."
            if not has_api_key
    ...
    with st.container(border=True, key="step6_agent_shell"):
        st.markdown(
            ...
            f'<div class="step6-heading" style="font-size:1.14rem;">{workspace_title}</div>'
            f'<div class="step6-subheading">{workspace_subheading}</div>',
            unsafe_allow_html=True,
        )
        ...
        if mode == "internal":
            ...
            preview_title = "Connected Preview"
            preview_copy = (
                "Governance gates have passed and the approved synthetic payload is ready. Add an API key to activate Connected Agent, or stay in Local mode."
                if not has_api_key
                else "Connected mode is selected. Activate Connected Agent to begin live deeper analysis on the approved synthetic payload."
            )
            st.markdown(
            ...
            if not has_api_key:
                if preview_cols[0].button("Connect API key", use_container_width=True, type="primary", key="step6_preview_connect"):
                    st.session_state.step6_open_runtime = True
                    st.rerun()
                if preview_cols[1].button("Return to Local mode", use_container_width=True, key="step6_preview_back_local"):
                    activate_local_mode()
            else:
                if preview_cols[0].button("Activate Connected Agent", use_container_width=True, type="primary", key="step6_preview_open_runtime"):
                    st.session_state.step6_open_runtime = True
                    st.rerun()
